...
echo '6.'
cat data-hold/legislators-current.json | jq '.[].bio.religion' | tr -d '"' | sort | uniq -c | sort -r
echo '7.'
...
echo '17.'
cat data-hold/congress-twitter-profiles.json | jq '.[] .followers_count' | sort -n | tail -n 1
echo '18.'
